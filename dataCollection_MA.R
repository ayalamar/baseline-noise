...
detrendData <- function(groupData) {
  ...
  ## maria - get IDs for cntrl and aging dfs
  Rcntrl_ppid <- Rcntrl %>%
  ...
  ## maria - put IDs back onto cntrl and aging dfs
  cntrl <- left_join(cntrl, Rcntrl_ppid, by = "participant")
  aging <- left_join(aging, Raging_ppid, by = "participant")
  
  ## maria - save detrended data with preserved IDs
  write.csv(Rcntrl,'Rcntrl.csv', row.names = F)
  write.csv(Raging,'Raging.csv', row.names = F)
  write.csv(cntrl,'cntrl.csv', row.names = F)
  write.csv(aging,'aging.csv', row.names = F)
...
saveDetrendedData <- function() {
  ...
  write.csv(dfs[['aligned']][['young']], file='data/young_localization_MA.csv', quote=F, row.names=F)
  write.csv(dfs[['aligned']][['aging']], file='data/aging_localization_MA.csv', quote=F, row.names=F)

  write.csv(dfs[['rotated']][['young']], file='data/young_rotated_localization_MA.csv', quote=F, row.names=F)
  write.csv(dfs[['rotated']][['aging']], file='data/aging_rotated_localization_MA.csv', quote=F, row.names=F)
